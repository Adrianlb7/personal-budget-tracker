...
add column payment_method text not null default 'external_expense'
check (payment_method in ('external_expense', 'savings_reimbursement'));

update public.recurring_commitments
set payment_method = 'savings_reimbursement'
where kind = 'external_installment' and destination_account_id is not null;
...
begin
  ...
  if new.kind = 'external_installment' and new.payment_method = 'savings_reimbursement'
     and new.destination_account_id is null then
    raise exception 'Savings reimbursement destination required';
  end if;
...
begin
  ...
  if commitment.kind = 'subscription' then
    ...
    if commitment.payment_method = 'savings_reimbursement' then
